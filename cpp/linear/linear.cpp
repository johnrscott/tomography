/************************************************************
* Title: Linear estimator using Qpp!
*
* Date created: 13th August 2018
*
* Language:    Cpp
*
* Overview:    Attempt to port the linear estimator
*              to use Qpp
*
...
int main() {
  ...
  cmat random_rho = randrho(2);
  cmat x_plus_state(2,2);
  x_plus_state << 0.5, 0.5,
                  0.5, 0.5;
  cmat x_minus_state(2,2);
  x_minus_state << 0.5, -0.5,
                  -0.5, 0.5;
  cmat y_plus_state(2,2);
  y_plus_state << 0.5,         cplx(0,-0.5),
                  cplx(0,0.5), 0.5;
  cmat y_minus_state(2,2);
  y_minus_state << 0.5,         cplx(0,0.5),
                   cplx(0,-0.5), 0.5;
  cmat z_plus_state(2,2);
  z_plus_state << 1, 0,
                  0, 0;
  cmat z_minus_state(2,2);
  z_minus_state << 0, 0,
                  0, 1;
  ...
  meas_y << 1,         cplx(0,1),
            cplx(0,1), 1;
  meas_y = std::sqrt(0.5) * meas_y;

  auto measured_X = measure(y_plus_state, gt.H, {0});
  auto measured_Y = measure(y_plus_state, meas_y, {0});
  auto measured_Z = measure(y_plus_state, gt.Id2, {0});
  
  // Print it!
  std::cout << "X result: " << std::get<0>(measured_X) << '\n';
  std::cout << ">> Probabilities: ";
  std::cout << disp(std::get<1>(measured_X), ", ") << '\n';

  std::cout << "Y result: " << std::get<0>(measured_Y) << '\n';
  std::cout << ">> Probabilities: ";
  std::cout << disp(std::get<1>(measured_Y), ", ") << '\n';

  std::cout << "Z result: " << std::get<0>(measured_Z) << '\n';
  std::cout << ">> Probabilities: ";
  std::cout << disp(std::get<1>(measured_Z), ", ") << '\n';
  ...
  */
  ...
  for(int k=0; k<M; k++) {
    ...
    for(int n=0; n<N; n++) {
      ...
      cmat D(2,2); D << x, 0,
		        0, 1-x;
      cmat U = randU(2); // random basis on 1 qubit
      cmat dens = U * D * U.adjoint(); // Maybe apply as a channel?
	...
	auto measured_X = measure(dens, gt.H, {0});
	auto measured_Y = measure(dens, meas_y, {0});
	auto measured_Z = measure(dens, gt.Id2, {0});
	...
	// It gives 0 when the result is the +1 outcome
	// and 1 if it is the -1 outcomes. So subtract
	// twice the result from 1
	X_data[k] = 1 - 2*std::get<0>(measured_X);
	Y_data[k] = 1 - 2*std::get<0>(measured_Y);
	Z_data[k] = 1 - 2*std::get<0>(measured_Z);
      ...
      */    
      ...
      //
      cmat dens_est = linear_estimate_XYZ(X_data, Y_data, Z_data, S);
      //cmat dens_est = enm_estimate_XYZ(X_data, Y_data, Z_data, S);
